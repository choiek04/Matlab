...
load fisheriris;
%%각 종을 숫자를 표현 1: versicolor, 2: virginica
spcs2num = [];
...
%평가데이터. 51~70ㅣ versicolor, 101~120: virginica 총 40개
...
tr_id = [51:1:70 101:1:120];
Test_data = meas(tr_id, 1:2);
Test_label = spcs2num(tr_id,:);

%%매트랩 내부 함수를 이용한 KNN 모델 만들기 (학습: 결정해야 할 것: k, 거리를
%%어떤 방법으로 할지)
k = 3; %인접한 이웃 3개를 보겠다.
...
%거리를 계산하는 default 방법은 유클리디안 이므로, 이렇게 작성해도 됨
